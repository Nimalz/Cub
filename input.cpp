...
// game pad用設定値
#define DEADZONE		4500			// 各軸の25%を無効ゾーンとする
#define RANGE_MAX		5000			// 有効範囲の最大値
#define RANGE_MIN		-5000			// 有効範囲の最小値
...
HRESULT UpdateKeyboard(void)
{
	...
	if(SUCCEEDED(hr))
	{
		...
		{
			g_keyStateTrigger[cnt] = ~keyStateOld[cnt] & g_keyState[cnt];
			g_keyStateRelease[cnt] = (keyStateOld[cnt] ^ g_keyState[cnt]) & ~g_keyState[cnt];
			g_keyStateRepeat[cnt] = (keyStateOld[cnt] ^ g_keyState[cnt]) & g_keyState[cnt];
