...
//UINT64 g_PausedPosition = 0;								// オーディオ現在位置（pause用）
BOOL g_Paused = FALSE;										// オーディオポーズしてるか？
...
void PauseSound(int label)
{
	...
	{// 再生中
		...
		g_Paused = TRUE;
		//g_PausedPosition = xa2state.SamplesPlayed;
...
void Sampleratedown(int label, float mul)
{
	...
	g_apSourceVoice[label]->Stop();
	g_apSourceVoice[label]->SetSourceSampleRate((UINT32)44100 * (UINT32)mul);
...
void SetVolume(void)
{
	for (int x = 0; x < 4; x++)
	{
		g_apSourceVoice[x]->SetVolume(1.0 * volume.master * volume.BGM);
	}

	for (int x = 4; x < SOUND_LABEL_MAX; x++)
	{
		g_apSourceVoice[x]->SetVolume(1.0 * volume.master * volume.SE);
	}
}